    ...
    <div style={{ maxWidth: '780px', margin: '2rem auto' }}>
      <div className="glass-card">
        <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: '1.8rem', marginBottom: '0.5rem' }}>
          📄 Upload Old Medical Documents / Reports
        </h2>
        <p style={{ color: 'var(--text-muted)', marginBottom: '1.5rem' }}>
          Upload previous prescriptions, lab results, or hospital summaries. Groq Vision will automatically extract diagnoses, medications, and dates.
        </p>
        ...
        <form onSubmit={handleUploadSubmit} style={{ marginBottom: '2rem' }}>
          ...
          >
            ...
            <div style={{ fontWeight: 600, fontSize: '1.1rem', color: '#f1f5f9' }}>
              {selectedFile ? selectedFile.name : 'Click or Drag & Drop Medical Document / Prescription Image'}
            </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem' }}>
            {selectedFile && (
              <button className="btn btn-primary" type="submit" disabled={isLoading}>
                {isLoading ? 'Extracting with Groq Vision...' : 'Upload & Extract Document →'}
              </button>
            )}
          </div>
        </form>
        ...
        {uploadedDocs.length > 0 && (
          <div style={{ marginTop: '2rem' }}>
            <h3 style={{ fontFamily: 'var(--font-heading)', fontSize: '1.3rem', marginBottom: '1rem', color: 'var(--accent-cyan)' }}>
              ✅ Extracted Document Evidence ({uploadedDocs.length})
            </h3>
                ...
                return (
                  <div key={idx} style={{
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', fontSize: '0.9rem' }}>
                      <div>
                        <strong style={{ color: 'var(--text-muted)' }}>Diagnoses Found:</strong>
                      ...
                      <div>
                        <strong style={{ color: 'var(--text-muted)' }}>Medications Found:</strong>
                        <ul style={{ paddingLeft: '1.2rem', marginTop: '0.25rem', color: '#cbd5e1' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '2.5rem', paddingTop: '1.5rem', borderTop: '1px solid var(--border-color)' }}>
          <span style={{ fontSize: '0.9rem', color: 'var(--text-subtle)', alignSelf: 'center' }}>
            {uploadedDocs.length === 0 ? 'No documents uploaded yet (Optional)' : `${uploadedDocs.length} document(s) uploaded`}
          </span>

          <button className="btn btn-primary" onClick={onNext}>
            Proceed to Final Record Review →
          </button>
        </div>
      </div>
    </div>
